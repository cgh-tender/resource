...
public class FileUtil {
    ...
    public static void download(HttpServletResponse response, String path,String fileName) {
        ...
        try {
            if (StringUtils.isBlank(fileName)){
                fileName = path.split(Matcher.quoteReplacement(File.separator))[path.split(Matcher.quoteReplacement(File.separator)).length-1];
            }else {
                if (!fileName.contains(NO_FILE_NAME_SEPARATOR)){
                    String end = fileName.split(FILE_NAME_SEPARATOR)[fileName.split(FILE_NAME_SEPARATOR).length - 1];
    ...
    public static void getImage(HttpServletResponse response,String path,Integer size) {
        ...
        try {
            ...
            if (size == null || size == 0){
                ...
                out = response.getOutputStream();;
                String end = path.split(FILE_NAME_SEPARATOR)[path.split(FILE_NAME_SEPARATOR).length - 1];
                FileUtil.getAndResizeImage(fis,out,size,end);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void getAndResizeImage(HttpServletResponse response,String path,Integer with,Integer height) {
        ...
        try {
            ...
            OutputStream out = response.getOutputStream();
            String end = path.split(FILE_NAME_SEPARATOR)[path.split(FILE_NAME_SEPARATOR).length - 1];
            getAndResizeImage(fis,out,with,height,end);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static String saveFileAndGetUrl(MultipartFile multipartFile,String toPath,String reName){
        if (null != multipartFile){
            ...
            if (StringUtils.isBlank(reName)){
                ...
                /*如果存在 . 则存在后缀*/
                if (reName.contains(NO_FILE_NAME_SEPARATOR)){
    ...
    public static boolean isImage(InputStream file){
        ...
        String byte50 = getFileHexString(b);
        Iterator<String> iterator = IMAGE_TYPE_MAP.keySet().iterator();
        while (iterator.hasNext()){
            if (byte50.toLowerCase().startsWith(iterator.next())){
                return true;
